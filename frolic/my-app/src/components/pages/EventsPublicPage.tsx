      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {events?.map((event) => (
            <div
              key={event.eventId}
              className="group flex flex-col bg-card rounded-2xl border border-border overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
            >
              {/* Image Area */}
              <div className="relative h-48 w-full bg-muted overflow-hidden">
                ...
                <div className="absolute top-4 right-4">
                  <Badge
                    ...
                    className={`font-semibold shadow-md ${event.status === "Open" ? "bg-emerald-500 hover:bg-emerald-600" : ""}`}
                  >
                    {event.status}
                  </Badge>
                </div>
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="mt-auto">
                  <Link to={`/events/${event.eventId}`} className="w-full block">
                    <button
                      className={`w-full py-2.5 rounded-lg font-medium transition-colors ${event.status === "Open"
